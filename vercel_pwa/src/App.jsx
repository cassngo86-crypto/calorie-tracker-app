...
export default function App() {
  ...
  const handleImageSelect = (e) => {
    ...
    reader.onloadend = () => {
      ...
      setCurrentAnalysis(null); // Clear previous analysis card
      setMealLabel(''); // Clear input
    };
  ...
  const handleAnalyzeAndSave = async () => {
    ...
    try {
      ...
      setSelectedImage(null); // Reset preview after successful save
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-800 pb-12 font-sans">
      <header className="bg-emerald-600 text-white p-4 shadow flex items-center justify-center gap-2">
        <span className="text-2xl">🥗</span>
        <h1 className="text-xl font-bold">NutriTrack PWA</h1>
      </header>

      <main className="max-w-xl mx-auto p-4 space-y-6">
        {/* Today's Calories Overview */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 text-center">
          <h2 className="text-gray-500 text-sm font-medium">Today's Total Intake</h2>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 space-y-4">
          ...
          {!selectedImage ? (
            <label className="border-2 border-dashed border-gray-300 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition">
              ...
              <span className="text-sm font-semibold text-gray-600">Choose or Take Photo</span>
            ...
            <div className="space-y-4">
              ...
              <div className="w-full h-64 bg-gray-900 rounded-lg overflow-hidden flex items-center justify-center">
                <img src={selectedImage} alt="Current Meal" className="max-h-full max-w-full object-contain" />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400 outline-none"
              ...
              <button
                ...
                className="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-3 rounded-lg shadow transition"
              >
        ...
        {/* Streamlit-Style Detailed Output Card */}
        {currentAnalysis && (
          <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 space-y-6">
            <div>
                  ...
                  <tbody className="divide-y divide-gray-200 text-gray-700">
                    {currentAnalysis.ingredients?.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="p-2">{item.weight_g}</td>
                        <td className="p-2">{item.protein_g}</td>
                        <td className="p-2">{item.carbs_g}</td>
                        <td className="p-2">{item.fat_g}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-3 p-3 bg-emerald-50 border border-emerald-100 rounded-lg text-xs font-semibold text-emerald-800">
            ...
            {currentAnalysis.health_benefits?.length > 0 && (
              <div>
                <h4 className="font-bold text-gray-800 text-sm mb-2 flex items-center gap-1">🌱 Health Benefits</h4>
            ...
            {currentAnalysis.cautions?.length > 0 && (
              <div>
                <h4 className="font-bold text-gray-800 text-sm mb-2 flex items-center gap-1">⚠️ Who Should NOT Consume (Caution)</h4>
                <ul className="list-disc list-inside text-xs text-gray-600 space-y-1">
                  {currentAnalysis.cautions.map((c, i) => <li key={i}>{c}</li>)}
                </ul>
              </div>
            )}
            ...
            {currentAnalysis.healthier_swaps?.length > 0 && (
              <div>
                <h4 className="font-bold text-gray-800 text-sm mb-2 flex items-center gap-1">💡 Healthier Swaps</h4>
                <ul className="list-disc list-inside text-xs text-gray-600 space-y-1">
                  {currentAnalysis.healthier_swaps.map((s, i) => <li key={i}>{s}</li>)}
                </ul>
              </div>
            )}
          </div>
        ...
        <div className="space-y-3">
          ...
          {meals.map((meal) => (
            <div key={meal.id} className="p-4 bg-white rounded-xl shadow-sm border border-gray-200 flex gap-4 items-center">
              {meal.image && (
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-3 gap-1 text-center text-[10px] bg-gray-50 p-1.5 rounded-lg border border-gray-100">
                  <div><span className="text-gray-400 block">Protein</span><span className="font-bold">{meal.protein}g</span></div>
                  <div><span className="text-gray-400 block">Carbs</span><span className="font-bold">{meal.carbs}g</span></div>
                  <div><span className="text-gray-400 block">Fat</span><span className="font-bold">{meal.fat}g</span></div>
                </div>
              </div>
            </div>
          ))}
